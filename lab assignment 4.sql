...
/*question 4*/
select PRO_NAME,PRO_ID from product where PRO_ID in 
(select PRO_ID from supplier_pricing where PRICING_ID in 
(select PRICING_ID from `order` where CUS_ID=2));

/*question 8*/
select CUS_NAME,CUS_GENDER from customer where CUS_NAME like "A%" or CUS_NAME like "%A";

/*question 5*/
 select * from supplier where supp_id in (select supp_id from supplier_pricing group by supp_id having
count(supp_id)>1);
...
/*question 9*/
call `order-directory`.supplierid_name_rating_service();
